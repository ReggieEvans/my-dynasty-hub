...
export function RatingPill({ value, size = 'md', variant = 'pill' }: RatingPillProps) {
...
function getColor(value: number | undefined, variant: 'pill' | 'text') {
  if (value === undefined || value < 65) return variant === 'pill' ? 'bg-red-600' : 'text-red-600'
  if (value < 70) return variant === 'pill' ? 'bg-orange-500' : 'text-orange-500'
  if (value < 80) return variant === 'pill' ? 'bg-yellow-500' : 'text-yellow-500'
